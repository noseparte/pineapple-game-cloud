package com.xmbl.mongodb.dao;

import com.xmbl.mongodb.util.Pagination;
import lombok.Data;
...
public abstract class GeneralDaoImpl<T> implements GeneralDao<T> {
    ...
    public Pagination<T> findPaginationByQuery(Query query, int pageNumber,
                                               int pageSize) {
        if (query == null) {
            query = new Query();
        }
        ...
        Pagination<T> page = new Pagination<T>(pageNumber, pageSize, totalCount);
