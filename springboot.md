...
management.health.couchbase.timeout = 1000ms ＃从服务器获取Bucket信息的超时时间。
...
management.health.status.http-mapping =＃将健康状态映射到HTTP状态码。默认情况下，已注册的运行状况映射到合理的默认值（例如，UP映射到200）。
...
management.metrics.distribution.sla。* = ＃以指定名称开头的仪表ID的特定SLA边界。最长的匹配获胜，键“all”也可用于配置所有仪表。
...
management.metrics.export.statsd.flavor = 要使用的
 datadog ＃StatsD线路协议。management.metrics.export.statsd.host= localhost ＃StatsD服务器的主机以接收导出的指标。
